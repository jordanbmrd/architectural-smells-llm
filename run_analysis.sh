...
cd /Users/polguillou/Documents/z-transformers
...
for version in */; do
    if [ -d "$version" ]; then
        ...
        if [ ! -f "code_quality_config.yaml" ]; then
            cp /Users/polguillou/Desktop/DMU-Work/architectural-smells-llm/code_quality_config.yaml .
        fi
        analyze_code_quality ./ --type architectural
        cd ..
    fi
done